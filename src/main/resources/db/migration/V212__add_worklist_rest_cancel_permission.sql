...
SELECT m.id, 'pacs.worklist.cancel', 'Worklist (Cancel)', 'ACTION', 'CANCEL', 8, 1, NOW()
FROM modules m
WHERE m.code = 'pacs-worklist'
...
VALUES ('POST', '/worklists/*/cancel', 'pacs.worklist.cancel', 'pacs.api', 1)
...
FROM roles r
JOIN module_details md ON md.code = 'pacs.worklist.cancel'
WHERE r.is_active = 1
  AND (
